...
struct profile_block
{
    ...
    ~profile_block(void)
    {
        ...
        bool recursive = (ParentIndex && (ParentIndex == GlobalProfilerParent));
        ...
        if (recursive)
           Anchor->TSCElapsedRecursive += Elapsed;
...
static void PrintTimeElapsed(u64 TotalTSCElapsed, profile_anchor *Anchor)
{
    ...
    {
        f64 PercentWithChildren = 100.0 * ((f64)(Anchor->TSCElapsed - Anchor->TSCElapsedRecursive) / (f64)TotalTSCElapsed);
